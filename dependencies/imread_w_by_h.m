function [X, map, alpha] = imread_w_by_h(varargin)
%Modified by Gavin Taylor, 2017. MIT License
...
if (isempty(fmt_s))
    ...
    %%%ok if error occurs here it is because matlab can't get to the above
    %%%private function now that this is out of the core directory
    %%%will need to add file format as second arg i.e. 'TIFF'
...
%--------------------------------------------------------------------------
function filename = get_full_filename(fid, errmsg, filename)
if (fid == -1)

    if ~isempty(dir(filename))
        % String 'Too many open files' is from strerror.
        % So, no need for a message catalog.
        if ~isempty(strfind(errmsg, 'Too many open files'))            
